// js/ui.js
// --- IMPORTS ---
import { getHitZone, evaluateCalculationStatus } from '../shared-logic.js';
import * as config from './config.js';
import * as state from './state.js';
// Import analysis functions that the UI will trigger
import { runAllAnalyses, handleStrategyChange, handleHistoricalAnalysis, updateActivePredictionTypes, labelHistoryFailures, calculateTrendStats } from './analysis.js';
...
const outerCircle = document.createElementNS("[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)", "circle");
...
config.rouletteWheel.forEach((number, index) => {
    ...
    const circle = document.createElementNS("[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)", "circle");
...
[...state.history].sort((a, b) => b.id - a.id).forEach(item => {
    ...
    li.innerHTML = `
        ...
        <div class="flex items-center space-x-2">
            <div class="status-box fail-box" style="display:${showFailIcon ? 'flex' : 'none'};"><svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></div>
            <div class="status-box success-box" style="display:${showSuccessIcon ? 'flex' : 'none'};"><svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg></div>
            <button class="delete-btn" data-id="${item.id}" aria-label="Delete item"><svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m-1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
        </div>
        ${aiDetailsHtml}
    `;
...
runAllAnalyses(winningNumber, newHistoryItem.id);
renderHistory();
...
const winningNumberInput = document.getElementById('winningNumberInput'); // Corrected ID from your HTML
...
const historyItem = state.history.find(item => item.id === calcId);
if (!historyItem) {
    console.error('History item not found for confirmation.');
    return;
}

// Update the history item with the result
evaluateCalculationStatus(historyItem, winningNumber, state.useDynamicTerminalNeighbourCount, state.activePredictionTypes, config.terminalMapping, config.rouletteWheel);
...
// Re-run analyses, render history, and save state
runAllAnalyses(winningNumber);
renderHistory();
drawRouletteWheel(historyItem.difference, winningNumber);
state.saveState();

// Check if the AI needs to be retrained
const successfulHistoryCount = state.history.filter(item => item.status === 'success').length;
if (aiWorker && successfulHistoryCount >= config.AI_CONFIG.trainingMinHistory) {
    updateAiStatus('AI Model: Training...');
...
window.handleConfirmWinningNumber = handleConfirmWinningNumber;
