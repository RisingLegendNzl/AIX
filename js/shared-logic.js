...
export function getRecommendation(context) {
    const {
        ...
        useTableChangeWarningsBool, rollingPerformance, // ADDED: Table Change Warning toggles and data
    ...
    if (useTableChangeWarningsBool && rollingPerformance && rollingPerformance.totalPlaysInWindow >= effectiveStrategyConfig.WARNING_MIN_PLAYS_FOR_EVAL) { //
        ...
        if (tableChangeDetected) { //
            ...
            // For 'Avoid Play', we ensure bestCandidate is null as no recommendation to bet is given
            bestCandidate = null; //
            finalHtml = `<strong class="${signalColor}">${signal}</strong> <br><span class="text-xs text-gray-600">Final Score: ${details.finalScore.toFixed(2)}</span><br><span class="text-xs text-gray-500">${reason}</span>`; //
            return { html: finalHtml, bestCandidate: null, details: details }; //
        }
    }
